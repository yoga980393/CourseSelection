...
struct FavoritesView: View {
    ...
    private var SelectedcourseBlocks: some View {
        ForEach(CoursesAlreadyInTheSchedule) { course in
            SelectedcourseBlocksForCourse(course: course)
        }
    }
    ...
    private func SelectedcourseBlocksForCourse(course: Course) -> some View {
        let sortedSchedules = course.schedule.sorted()
        var startSchedule = sortedSchedules[0]
        var endSchedule: Int?
        var courseBlocks = [AnyView]()
    ...
    private func SelectedCourseBlock(course: Course, startSchedule: Int, endSchedule: Int?, showText: Bool) -> some View {
        let day = (startSchedule / 100) - 1
        let classIndex = (startSchedule % 100) - 1
        let padding: CGFloat = 2
